...
import com.ecommerce.rates_service.dto.RatePatchDTO;
...
public class RateController {
    ...
    @PostMapping
    @Operation(summary = "Create a new rate", description = "Creates a new rate with the provided details")
    @ApiResponses(value = {
    ...
    @PatchMapping("/{id}")
    @Operation(summary = "Update rate partially", description = "Updates the price of an existing rate")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Rate updated successfully"),
            @ApiResponse(responseCode = "404", description = "Rate not found")
    })
    public ResponseEntity<RateResponseDTO> patchRate(@PathVariable Long id, @RequestBody RatePatchDTO patchDTO) {
        RateResponseDTO response = rateService.updateRatePrice(id, patchDTO.getPrice());
        return ResponseEntity.ok(response);
    }
